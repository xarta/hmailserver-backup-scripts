...
' TODO: move the schedule settings to Xarta.json and use For Each iteration
If (WScript.Arguments.Count = 0) Then
	SetScheduler GetXartaJsonObject(XartaScriptDir), 	"hMailServer Settings", _
														"hmsettings", _
														"WEEKLY", _
														"MON,TUE,WED,THU,FRI,SAT,SUN", _
														"01:00"
	WScript.Sleep 100 ' allow SchTasks time to add task
	SetScheduler GetXartaJsonObject(XartaScriptDir), 	"hMailServer mySql Dump", _
														"hmmysqldump", _
														"WEEKLY", _
														"MON,TUE,WED,THU,FRI,SAT,SUN", _
														"01:15"
	WScript.Sleep 100 ' allow SchTasks time to add task
	SetScheduler GetXartaJsonObject(XartaScriptDir), 	"hMailServer 7zip data", _
														"hm7zipdata", _
														"WEEKLY", _
														"MON,TUE,WED,THU,FRI,SAT,SUN", _
														"01:25"
else
	If (WScript.Arguments(0) = "hmsettings") Then
		BkUpHMSsettings GetXartaJsonObject(XartaScriptDir)
	ElseIf (WScript.Arguments(0) = "hmmysqldump") Then
		BkUpMySql GetXartaJsonObject(XartaScriptDir), XartaScriptDir
	ElseIf (WScript.Arguments(0) = "hm7zipdata") Then
...
Sub SetScheduler(o, taskName, taskArg, SC, D, ST)
	...
	' THIS DELETE INSTRUCTION NO LONGER WORKS???  NEED TO CHECK.
