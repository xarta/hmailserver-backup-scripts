...
On error resume Next
    ...
    If objFSO.FileExists(o("paths")("mysqldumpdefaultsextrafile")) Then
        Set objFile = objFSO.OpenTextFile(o("paths")("mysqldumpdefaultsextrafile"), ForWriting)
    Else
        Set objFile = objFSO.CreateTextFile(o("paths")("mysqldumpdefaultsextrafile"), ForWriting)
    End If
    ...
    objFile.Writeline("")
    objFile.Writeline("[mysql]")
    objFile.Writeline("user=" & o("mySQL")("hmailserver")("User"))
    objFile.Writeline("password=" & chr(34) & o("mySQL")("hmailserver")("Password") & chr(34))
    objFile.Close

    If err.Number <> 0 Then
        call logging.write("WriteSqlBackUpScripts error: " & err.Description,3)
        call logging.write("WriteSqlBackUpScripts error: " & o("paths")("mysqldumpdefaultsextrafile"),3)
    End If
End Sub
...
On error resume Next
    ...
    ' Create mySQL User "dump" if the User does not already exist,
    ' just with back-up priviledges for mysqldump
    ' ASSUMES mySQL User "hMailServer" already exists with grant priviledges
    ' and priviledges for hmailserver database schema
    ' TODO: Script to add all the users, asking for root credentials
    Dim mysqlPath, defaultsFile, optionUser, backupUser, backupPassword, database

    mysqlPath =      " -mysqlPath " & """'" & o("paths")("mysqlexe") & "'"""
    defaultsFile =   " -defaultsFile " & o("paths")("mysqldumpdefaultsextrafile")
    optionUser =     " -optionUser '-u" & o("mySQL")("hmailserver")("User") & "'"
    backupUser =     " -backupUser dump"
    backupPassword = " -backupPassword " & """'" & o("mySQL")("backup")("Password") & "'"""
    database =       " -database hmailserver"


    Dim RetVal
    Dim script, args, waitReturn

    script = XartaScriptDir & "XartaMySqlCreateUsers.ps1"
    args = mysqlPath & defaultsFile & optionUser & backupUser & backupPassword & database
    waitReturn = True
